...
> **This is a sample.** It exists so the lab note layout can be previewed in `npm run dev`. It does not describe a real lab session, and it is excluded from production builds by `draft: true`.
...
Sample text. This section records the actual steps, with commands and config snippets.

```text
$ example-command --sample   # sample output block, not a real command run
```
...
Sample text. A real note closes with loose ends, follow-ups, and links to related notes or the project page.
